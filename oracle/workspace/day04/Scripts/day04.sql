 ...
 * 기존의 테이블을 그대로 놔둔채 필요한 컬럼들 및 새로운 컬럼들을 만든 *가상 테이블*
 * 실제 데이터가 저장되는 것은 아니지만 VIEW 를 통해서 데이터를 관리할 수 있다.
 * - 독립성 : 다른 곳에서 접근하지 못하도록 하는 성질 SELECT 절에서 관리할 것만 적음
 * - 편리성 : 매번 길고 복잡한 쿼리문을 작성할 필요가 없다.
...
SELECT * FROM VIEW 
/* 임플로이 테이블에서 사원 이름과 그 사원의 매니저 이름이 있는 VIEW 만들기 */
CREATE VIEW NAME AS
SELECT E1.FIRST_NAME || LAST_NAME EMPLOYEES_NAME , E2.FIRST_NAME ||E2.LAST_NAME MANAGER_NAME
FROM EMPLOYEES E1
JOIN EMPLOYEES E2
